...
import com.grupo53.tienda53.DTO.VentaClientesVO;
...
public class VentaDAO {
	...
	public int contadorVentas() {
		...
		try {
			...
			PreparedStatement consulta = conex.getConnection()
					.prepareStatement("SELECT MAX(codigo_venta)" + "FROM ventas;");
	...
	public ArrayList<VentaClientesVO> listaDeVentasCliente() {
		// lista que contendra el o los usuarios obtenidos
		ArrayList<VentaClientesVO> listaventasclientes = new ArrayList<VentaClientesVO>();
		...
		try {
			// prepare la sentencia en la base de datos
			PreparedStatement consulta = conex.getConnection().prepareStatement("SELECT clientes.cedula_cliente, clientes.nombre_cliente, ventas.valor_venta FROM clientes JOIN ventas ON clientes.cedula_cliente = ventas.cedula_cliente;");
			...
			// entidad con los datos encontrados
			while (res.next()) {
				VentaClientesVO Venta = new VentaClientesVO();
				Venta.setCedula_cliente(Integer.parseInt(res.getString("cedula_cliente")));
				Venta.setNombre_cliente(res.getString("nombre_cliente"));
				Venta.setValor_venta(Double.parseDouble(res.getString("valor_venta")));
